...
<div class="container">
    <div class="col-sm-12">
        ...
        if(isset($_GET['idbangnhap'])){
            $id = $_GET['idbangnhap'];
        $sql1 = "SELECT * FROM bangnhapgdn WHERE id = '$id'";
        ...
        <div class="row">
            ...
            <form method="post" action="exe/editformgdn.php">
            <div class="col-sm-4">
            ...
            <input type="number" name="tongsokhachtuvan" class="form-control" placeholder="Nhập tổng số khách tư vấn" value="<?php echo $row['tongsokhachtuvan']; ?>" disabled>
                <br>
            <label>Không hiệu quả :</label><span class="luuy">(Thông số thể hiện số lượng không hiệu quả)</span>
            <input type="number" name="khonghieuqua" class="form-control" placeholder="Nhập số không hiệu quả" value="<?php echo $row['khonghieuqua']; ?>" disabled>
            <br>
            <label>Đến khám :</label><span class="luuy">(Thông số thể hiện số lần đặt hẹn)</span>
            <input type="number" name="denkham" class="form-control" placeholder="Nhập số lần đến khám" value="<?php echo $row['denkham']; ?>" disabled>
            </div>
            <div class="col-sm-4">
            ...
            <input type="number" name="hieuqua" class="form-control" placeholder="Nhập số hiệu quả" value="<?php echo $row['hieuqua']; ?>" disabled>
                <br>
            <label>Đặt hẹn :</label><span class="luuy">(Thông số thể hiện số lần đặt hẹn)</span>
            <input type="number" name="dathen" class="form-control" placeholder="Nhập số lần đặt hẹn" value="<?php echo $row['dathen']; ?>" disabled>
                <br>
                <label>Unique Visitors :</label><span class="luuy">(Thông số thể hiện tổng số lần hiển thị LIVECHAT)</span>
            <input type="number" name="uniquevisitors" class="form-control" placeholder="Nhập số lần hiện livechat" value="<?php echo $row['uniquevisitor']; ?>" disabled>
                <br>
            </div>
            <div class="col-sm-4">
            <label>Ghi chú :</label><span class="luuy" style="padding-right:20px;">(Ghi chú thêm)</span>
            <textarea name="ghichu" class="form-control" style="width:95%;height:200px;" placeholder="Nhập ghi chú" disabled><?php echo $row['ghichu']; ?></textarea>
                <br>
